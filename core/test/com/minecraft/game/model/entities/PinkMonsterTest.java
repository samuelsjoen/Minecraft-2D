...
class PinkMonsterTest {
    ...
    void pinkMonsterShouldTransitionToDeadStateWhenHealthDepletes() {
        // Damage the pinkMonster enough times to deplete health
        when(mockBody.getLinearVelocity()).thenReturn(new Vector2(0, 0));
        pinkMonster.getHit(3); // Assuming one hit deals 1 damage
        pinkMonster.update(0.1f); // Simulate an update call with deltaTime
        assertEquals(PinkMonster.State.DEAD, pinkMonster.getCurrentState(),
                "pinkMonster should transition to DEAD state");
    }

    @Test
    void pinkMonsterShouldFaceLeftWhenPlayerIsOnTheLeft() {
        // Place player to the left of the pinkMonster
        when(mockPlayer.getBody().getPosition()).thenReturn(new Vector2(5.5f, 5.0f)); // Player is now to the left
        when(mockBody.getLinearVelocity()).thenReturn(new Vector2(0, 0));
        // System.out.println("pinkMonster position: " +
        // pinkMonster.getBody().getPosition().x);
        // System.out.println("Player position: " +
        // mockPlayer.getBody().getPosition().x);
        // System.out.println("pinkMonster facing right: " +
        // pinkMonster.isFacingRight());
        ...
        assertTrue(pinkMonster.isFacingRight(), "pinkMonster should be facing left when player is to the left");
    }

    @Test
    void pinkMonsterShouldTransitionToAttackingWhenCloseToPlayer() {
        // Place player close enough to trigger attacking state
        when(mockPlayer.getBody().getPosition()).thenReturn(new Vector2(5.5f, 5.0f)); // Close to the pinkMonster
        when(mockBody.getLinearVelocity()).thenReturn(new Vector2(0, 0));

        pinkMonster.setAttackFrameTrue(); // Simulate conditions for attacking
        pinkMonster.update(0.1f); // Simulate an update call with deltaTime
